...
class Blue_Filter
{
        ...
        foreach($filter as $k => $v){
            if(isset($var[$k]) == false){
                if($v[3] === true){
                    ...
                    throw new Blue_Exception_Warning("参数错误", array('k' => $k));
                }
            }
            if(!is_array($v[1])){
                $v[1] = array();
            ...
            if(method_exists(__CLASS__, $v[0]) == false){
                throw new Blue_Exception_Fatal("filter {$v[0]} is not exist");
            }
            ...
            if(null === $rv){
                ...
                throw new Blue_Exception_Warning($msg, array('k' => $k, 'v' => $var[$k]));
            }
            $ret[$k] = $rv;
        }
        return $ret;
    ...
    public static function filterIndexedArray($filterMethod, $var, $isIgnore = false){
        ...
        if(method_exists(__CLASS__, $filterMethod) == false){
            throw new Blue_Exception_Fatal("filter {$filterMethod} is not exist");
        }
        $ret = array();
        foreach($var as $v){
            ...
            if(null === $rv){
                if($isIgnore == false){
                    throw new Blue_Exception_Warning("value:{$v} is not match");
                }
            }else{
                $ret[] = $rv;
            }
