<?php
/*********************************************************************
 filename    : nusoap.class.php
 description : 远程条用接口类
 author      : zhurongqiang
 version     : ver 1.0.0 beta1
 contact     : zhurongqiang1@126.com
 create      : 2014-12-25 13:48:55
**********************************************************************/
...
class nusoap_base {
    ...
    /**
     * Current error string (manipulated by get_error/set_error)
	 *
	 * @var string
	 * @access private
	 */
    ...
    */
	function getNamespaceFromPrefix($prefix){
		...
		//$this->set_error("No namespace registered for prefix '$prefix'");
		return false;
	}
...
class nusoap_xmlschema extends nusoap_base  {
	...
	*/
	function parseString($xml,$type){
		...
		if($xml != ""){
			...
			$this->set_error('no xml passed to parseString()!!');
		}
	}
	...
	*	... and so on (see add_complex_type)
	*	)
	*
	...
	* @see getTypeDef
	*/
	function add_complex_type($name,$typeClass='complexType',$phpType='array',$compositor='',$restrictionBase='',$elements=array(),$attrs=array(),$arrayType=''){
		...
		$this->xdebug("add_complex_type $name:");
	...
	*/
	function add_element($attrs) {
		...
		$this->xdebug("add_element " . $attrs['name']);
...
//			$this->set_error('cURL Extension, or OpenSSL extension w/ PHP version >= 4.3 is required for HTTPS');
			$this->set_error('The PHP cURL Extension is required for HTTPS or NLTM.  You will need to re-build or update your PHP to include cURL or change php.ini to load the PHP cURL extension.');
			return false;
		}
...
//		}
		...
		$this->set_error('Unknown scheme ' . $this->scheme);
	...
	function send($data, $timeout=0, $response_timeout=30, $cookies=NULL) {
		...
		while ($this->tryagain) {
			...
			if ($tries++ < 2) {
				...
				$this->set_error("Too many tries to get an OK response ($this->response_status_line)");
			}
		}		
	...
	function sendRequest($data, $cookies = NULL) {
	  ...
	  if ($this->io_method() == 'socket') {
		...
		if(!fputs($this->fp, $this->outgoing_payload, strlen($this->outgoing_payload))) {
			$this->set_error('couldn\'t write message data to socket');
	  ...
	  if ($this->io_method() == 'socket') {
	    ...
	    while (!isset($lb)){
			...
			if(feof($this->fp)) {
				...
				$this->set_error('server failed to send headers');
				return false;
			}
			...
			if ($tmplen == 0) {
				...
				$this->set_error('socket read of headers timed out');
				return false;
			}
		...
		do {
			if ($chunked) {
				...
				if ($tmplen == 0) {
					...
					$this->set_error('socket read of chunk length timed out');
					return false;
				}
		    ...
		    while (($strlen < $content_length) && (!feof($this->fp))) {
				...
				if (($tmplen == 0) && (!feof($this->fp))) {
					...
					$this->debug("read before timeout:\n" . $data);
					$this->set_error('socket read of body timed out');
					return false;
				}
				$strlen += $tmplen;
				$data .= $tmp;
			...
			if ($chunked && ($content_length > 0)) {
				...
				if ($tmplen == 0) {
					...
					$this->debug("read before timeout:\n" . $data);
					$this->set_error('socket read of chunk terminator timed out');
					return false;
				}
			}
		...
		if($this->incoming_payload == ''){
			$this->set_error('no response from server');
			return false;
		}
...
//				$this->set_error('Decoding of chunked data failed');
//				return false;
//			}
			//print "<pre>\nde-chunked:\n---------------\n$data\n\n---------------\n</pre>";
			// set decoded payload
...
//		}
		...
		if ($pos = strpos($data,"\r\n\r\n")) {
			...
			$this->set_error('no proper separation of headers and document');
			return false;
		}
		$header_data = trim(substr($data,0,$pos));
		$header_array = explode($lb,$header_data);
		...
		$http_reason = count($arr) > 2 ? $arr[2] : '';
 		...
 		if (isset($this->incoming_headers['www-authenticate']) && $http_status == 401) {
			...
			$this->set_error('HTTP authentication failed');
			return false;
 		}
		...
		if (
			...
			$this->set_error("Unsupported HTTP response status $http_status $http_reason (soapclient->response has contents of the response)");
			return false;
		}
    			...
    			if(function_exists('gzinflate')){
					...
					if ($this->incoming_headers['content-encoding'] == 'deflate') {
						if ($degzdata = @gzinflate($data)) {
	    					...
	    					$this->set_error('Error using gzinflate to inflate the payload');
	    				}
						...
						if ($degzdata = @gzinflate(substr($data, 10))) {	// do our best
							...
							$this->set_error('Error using gzinflate to un-gzip the payload');
	    				}
					}
					...
					$this->set_error('The server sent compressed data. Your php install must have the Zlib extension compiled in to support this.');
				}
			} else {
				$this->debug('Unsupported Content-Encoding ' . $this->incoming_headers['content-encoding']);
				$this->set_error('Unsupported Content-Encoding ' . $this->incoming_headers['content-encoding']);
			}
		} else {
		...
		if(strlen($data) == 0){
			...
			$this->set_error('no data present after HTTP headers');
			return false;
		}
...
class nusoap_server extends nusoap_base {
	...
	*/
	function parse_http_headers() {
		...
		if(function_exists('getallheaders')){
			...
			$this->set_error("HTTP headers not accessible");
		}
	}
	...
	*/
    function parseRequest($headers, $data) {
    	...
    	if (!isset($headers['content-type'])) {
			$this->set_error('Request not of type text/xml (no content-type header)');
			return false;
    	}
		if (!strstr($headers['content-type'], 'text/xml')) {
			$this->set_error('Request not of type text/xml');
			return false;
		}
	...
	function register($name,$in=array(),$out=array(),$namespace=false,$soapaction=false,$style=false,$use=false,$documentation='',$encodingStyle=''){
		...
		if(false == $soapaction) {
			if (isset($_SERVER)) {
				...
				$this->set_error("Neither _SERVER nor HTTP_SERVER_VARS is available");
			}
        	if ($HTTPS == '1' || $HTTPS == 'on') {
        		$SCHEME = 'https';
        	} else {
    ...
    */
    function configure_wsdl($serviceName,$namespace = false,$endpoint = false,$style='rpc', $transport = 'http://schemas.xmlsoap.org/soap/http', $schemaTargetNamespace = false)
    {
    	global $HTTP_SERVER_VARS;
...
class wsdl extends nusoap_base {
    ...
    function parseWSDL($wsdl = '') {
        ...
        if ($wsdl == '') {
            ...
            $this->set_error('no wsdl passed to parseWSDL()!!');
            return false;
        }
	...
	*/
	...
	function serializeRPCParameters($operation, $direction, $parameters, $bindingType = 'soap') {
		...
		if ($direction != 'input' && $direction != 'output') {
			...
			$this->set_error('The value of the \$direction argument needs to be either "input" or "output"');
			return false;
		} 
		if (!$opData = $this->getOperationData($operation, $bindingType)) {
			$this->debug('Unable to retrieve WSDL data for operation: ' . $operation . ' bindingType: ' . $bindingType);
			$this->set_error('Unable to retrieve WSDL data for operation: ' . $operation . ' bindingType: ' . $bindingType);
			return false;
		}
	...
	function serializeParameters($operation, $direction, $parameters)
	{
		...
		if (!$opData = $this->getOperationData($operation)) {
			...
			$this->set_error('Unable to retrieve WSDL data for operation: ' . $operation);
			return false;
		}
		$this->debug('opData:');
		$this->appendDebug($this->varDump($opData));
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
		...
		if(!$typeDef = $this->getTypeDef($uqType, $ns)){
			$this->set_error("$type ($uqType) is not a supported type.");
		...
		if (!isset($typeDef['phpType'])) {
			$this->set_error("$type ($uqType) has no phpType.");
		...
		if ($phpType == 'struct') {
			...
			if (is_array($value)) {
				...
				$this->set_error("phpType is struct, but value is not an array: see debug output for details");
				$xml = '';
			}
	...
	*/
	function add_complex_type($name,$typeClass='complexType',$phpType='array',$compositor='',$restrictionBase='',$elements=array(),$attrs=array(),$arrayType='') {
		...
		$this->schemas[$typens][0]->add_complex_type($name,$typeClass,$phpType,$compositor,$restrictionBase,$elements,$attrs,$arrayType);
	}
	...
	*/
	function add_element($attrs) {
		$typens = isset($this->namespaces['types']) ? $this->namespaces['types'] : $this->namespaces['tns'];
		$this->schemas[$typens][0]->add_element($attrs);
	}
	...
	*/
	function addOperation($name, $in = false, $out = false, $namespace = false, $soapaction = false, $style = 'rpc', $use = 'encoded', $documentation = '', $encodingStyle = ''){
		...
		if ($style == 'document') {
			...
			$this->add_complex_type($name . 'RequestType', 'complexType', 'struct', 'all', '', $elements);
			$this->add_element(array('name' => $name, 'type' => $name . 'RequestType'));
			...
			$this->add_complex_type($name . 'ResponseType', 'complexType', 'struct', 'all', '', $elements);
			$this->add_element(array('name' => $name . 'Response', 'type' => $name . 'ResponseType', 'form' => 'qualified'));
	...
	*/
	function nusoap_parser($xml,$encoding='UTF-8',$method='',$decode_utf8=true){
		...
		if(!empty($xml)){
			...
			$this->set_error('xml was empty, didn\'t parse!');
		}
	}
...
class nusoap_client extends nusoap_base  {
	...
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			if (is_string($params)) {
				...
				$this->set_error('params must be array or string');
				return false;
			}
			...
			if ($errstr = $this->wsdl->get_error()) {
				$this->debug('got wsdl error: '.$errstr);
				$this->set_error('wsdl error: '.$errstr);
				return false;
			}
			...
			$this->appendDebug($this->wsdl->getDebug());
			$this->wsdl->clearDebug();
			$this->set_error('operation '.$operation.' not present in WSDL.');
		...
		if($errstr = $this->get_error()){
			...
			if(is_array($return) && isset($return['faultcode'])){
				...
				$this->set_error($return['faultcode'].': '.$return['faultstring']);
				$this->fault = true;
	...
	*/
	function checkWSDL() {
		...
		if ($errstr = $this->wsdl->get_error()) {
			...
			$this->set_error('no operations defined in the WSDL document!');
		}
	}
	...
	*/
    function parseResponse($headers, $data) {
		...
		$this->appendDebug($this->varDump($headers));
    	if (!isset($headers['content-type'])) {
			$this->set_error('Response not of type text/xml (no content-type header)');
			return false;
    	}
		if (!strstr($headers['content-type'], 'text/xml')) {
			$this->set_error('Response not of type text/xml: ' . $headers['content-type']);
			return false;
		}
		if (strpos($headers['content-type'], '=')) {
			$enc = str_replace('"', '', substr(strstr($headers["content-type"], '='), 1));
